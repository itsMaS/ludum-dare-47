...
public class WormController : PlayerController
{
    ...
    {
        base.Update();
        rb.AddForce(movement*Config.pa.speed.Evaluate(level) * Time.deltaTime);
        ...
        for (int i = 1; i < Parts.Length; i++)
        {
            ...
            Parts[i].transform.position = Vector2.MoveTowards(Parts[i].position, Parts[i - 1].position, speed*Config.pa.jointFollowSpeed*Time.deltaTime);
        }
    }
